(ns clojure-llm.collect.email-groups.clojure
  (:require
...
(def base-page "https://www.mail-archive.com/clojure@googlegroups.com/")

(def first-page (str base-page "msg107207.html"))
...
(defn update-url [url]
  (let [base-count (if (str/starts-with? url "https://") (count base-page) 0)
        ...
        p (->> cleaned-url edn/read-string dec)
        p (if ends-with-hash? (dec p) p)
        p (if (@p-atom p) (dec p) p)]
    (swap! p-atom conj p)
    (when  (< 10000 p)
      (let [new-url (str "msg" p ".html")]
        new-url))))

(defprocessor :landing-page
  :cache-template "clojure-google-mail-group/:page"
...
(def sample-raw-data-file-name "data/clojure_mailgroup/clojure_llm_clojure_mailgroup_sample.json")
(def raw-data-file-name "data/clojure_mailgroup/clojure_llm_clojure_mailgroup.json")
(def data-file-name "data/clojure_mailgroup/clojure_llm_clojure_mailgroup_prompts.json")
(def sample-data-file-name "data/clojure_mailgroup/clojure_llm_clojure_mailgroup_prompts_sample.json")
